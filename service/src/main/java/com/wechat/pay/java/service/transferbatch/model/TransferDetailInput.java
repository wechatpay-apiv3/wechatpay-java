...
public class TransferDetailInput {
  ...
  /**
   * 收款用户姓名 说明：收款方姓名。支持标准RSA算法和国密算法，公钥由微信侧提供 明细转账金额<0.3元时，不允许填写收款用户姓名 明细转账金额 >=
  ...
  /** 收款用户身份证 说明：收款方身份证号，可不用填（采用标准RSA算法，公钥由微信侧提供） 当填入收款方身份证号时，姓名字段必须填入。 */
  @Encryption
  ...
  public TransferDetailInput cloneWithCipher(UnaryOperator<String> s) {
    ...
    if (userIdCard != null && !userIdCard.isEmpty()) {
      copy.userIdCard = s.apply(userIdCard);
    }
    return copy;
  }
}
